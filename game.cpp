...
int random_spikes[1000];
int random_bombs[100];
int random_coins[100];
int random_hearts[50];
int random_stars[25];
...
void init()
{
    ...
    menu_spike = buildModel("spike.obj");
    menu_spike->scale(0.25);
    menu_spike->setColor(0.0, 1.0, 0.0);

    // draw spikes and save their positions
    for (int i=0; i<g.num_spikes; i++) {
		spikes[i] = buildModel("spike.obj");
		spikes[i]->scale(0.25);
...
void physics()
{
	...
	if (countdown_done) {
		// move player
    	g.cameraPosition[0] += spot[0]*0.18;
    	g.cameraPosition[2] += spot[2]*0.18;
    }
...
void render()
{
	...
	unsigned int yellow = 0x000aaff11;
	...
	if (g.display_attract) {
	    ...
	    if (blinkon){
		    ...
		    i.left = g.xres/2;
		    i.center = g.yres/2;
		    ggprint13(&i, 20, yellow, "Press 'Enter' for menu");
	    }
		if (g.moveLeft && g.angle > -PI/8.0) {
			Vec newspot;
			y_rotate(-0.01, spot, newspot);
			spot[0] = newspot[0];
			spot[1] = newspot[1];
	...
	// Start Menu
		...
		r.left = g.xres/2 - 50;
		ggprint13(&r, 16, yellow, "START");
		ggprint13(&r, 16, yellow, "TUTORIAL");
		ggprint13(&r, 16, yellow, "AUDIO SETTINGS");
		ggprint13(&r, 16, yellow, "VIEW SCORES");

		r.bot = g.yres - g.yres/5 - 16*(g.menu_position-1);
		r.left = g.xres/2 - 75;
		ggprint13(&r, 16, yellow, ">>");
	...
	// Tutorial
		...
		r.bot = g.yres - (g.yres/5);
		r.left = g.xres/2 - 150;
		ggprint13(&r, 16, yellow, "Key controls:");
		ggprint12(&r, 16, yellow, "A - turn left");
		ggprint12(&r, 16, yellow, "D - turn right");
		ggprint12(&r, 16, yellow, "W - sprint");
		ggprint12(&r, 16, yellow, "Spacebar - jump");

		r.bot = g.yres - (g.yres/5);
		r.left = g.xres/2 + 50;
		ggprint13(&r, 16, yellow, "Items:");
		ggprint12(&r, 16, yellow, "Spike - hurts player (-25%% hp)");
		ggprint12(&r, 16, yellow, "Bomb - hurts player (-25%% hp)");
		ggprint12(&r, 16, yellow, "Coin - increases score");
		ggprint12(&r, 16, yellow, "Heart - replenishes health (+25%% hp)");
		ggprint12(&r, 16, yellow, "Star - invincibility for 3 seconds");
	...
	// Audio Settings
		...
		r.bot = g.yres - (g.yres/5);
		r.left = g.xres/2 - 50;
		ggprint13(&r, 16, yellow, "AUDIO ON");
		ggprint13(&r, 16, yellow, "AUDIO OFF");

		r.bot = g.yres - (g.yres/5) - 55;
		if (g.audio_on) {
			ggprint13(&r, 16, yellow, "Audio is currently: ON");
		} else {
			ggprint13(&r, 16, yellow, "Audio is currently: OFF");
		}
	...
	// Game Over Display
		...
		r.left = g.xres/2 - 60;
		r.center = 0;
		ggprint16(&r, 16, yellow, "Your score: %d", g.coins);
		blinkon = check_text_timer(blinkon);
	    blinkoff = check_notext_timer(blinkoff);
	    if (blinkon){
	    	Rect i;
	...
	// Game Over Menu
		...
		r.bot = g.yres - g.yres/5;
		r.left = g.xres/2 - 50;
		ggprint13(&r, 16, yellow, "PLAY AGAIN");
		ggprint13(&r, 16, yellow, "TUTORIAL");
		ggprint13(&r, 16, yellow, "AUDIO SETTINGS");
		ggprint13(&r, 16, yellow, "VIEW SCORES");
		r.bot = g.yres - g.yres/5 - 16*(g.menu_position-1);
	...
	// Game HUD
		...
		r.left = g.xres/2 - 75;
		r.center = 0;
		ggprint13(&r, 16, 0x000000, "%s", player_name);
		...
		// Display coin count
		r.bot = g.yres - (g.yres/5) - 20;
		r.left = g.xres/2 - 75;
		r.center = 0;
		ggprint13(&r, 16, 0x00998822, "Coins: %d", g.coins);
    }
    ...
    r.left = g.xres - 75;
    r.center = 0;
    ggprint10(&r, 16, 0x00998822, "fps: %.2f", fps);
...
void player1Name(char player_name[], Input &input)
{
    ...
    p.left = g.xres/2 - 60;
    p.center = 0;
    ggprint13(&p, 20, black, "Input your name:");

    p.bot = g.yres - g.yres/8 - 30;
    p.left = g.xres/2 - 60;
    p.center = 0;
    ggprint13(&p, 20, black, "%s", player_name);

    p.bot = g.yres - g.yres/8 - 60;
    p.left = g.xres/2 - 60;
    p.center = 0;
    ggprint13(&p, 20, black, "Press Enter to Play!");
